...
zenity --question --title="BZU GameMod Boosting Installer beta4-7" --text="Данный скрипт установит на вашу OC GNU/Linux утилиту bzu-gmb-beta4. Она поможет вам быстро и легко установить все, что требуется для оптимизации и ускорения системы для игр и программ, которые используют 3D графику. Внимание! Скрипт офицально поддерживает только Ubuntu 20.04-20.10 и оптимизирован под видеокарты компании AMD. Установку Вы совершаете на свой страх и риск, автор не несёт ответственности за любые негативные последствия для вашей OC. Утилита будет установлена в папку /usr/share/bzu-gmb. Нажмите Да, если готовы продолжить установку." --width=560 --height=128
if [ "$?" -eq "0" ];then

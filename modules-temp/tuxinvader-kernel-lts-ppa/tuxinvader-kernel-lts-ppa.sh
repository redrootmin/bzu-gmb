...
tput setaf 2; echo "Установка оригинальной LTS версии ядра Linux от tuxinvader [https://launchpad.net/~tuxinvader/+archive/ubuntu/lts-mainline-longterm]. Версия скрипта 1.0b, автор: Яцына М.А."
tput sgr0
...
echo "${pass_user}" | sudo -S add-apt-repository ppa:tuxinvader/lts-mainline-longterm -y || let "error += 1"
if echo "${linuxos_run_bzu_gmb}" | grep -ow "Mint" > /dev/null ;then
echo "${pass_user}" | sudo -S apt update -y
fi
echo "${pass_user}" | sudo -S apt install -f -y linux-generic-5.10* || let "error += 1"

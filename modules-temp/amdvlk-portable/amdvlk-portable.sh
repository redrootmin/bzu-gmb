...
tput setaf 2; echo "Установка открытого драйвера Vulkan - AMDVLK c поддержкой старых карт AMD GCN1 (HD7970+)[https://github.com/GPUOpen-Drivers/AMDVLK]. Установка открытого драйвера AMDVLK производиться в формате Portable. Версия скрипта 1.0b, автор: Яцына М.А."
tput sgr0
...
cd "/home/${USER}/.local/share/bzu-gmb-utils"
...
if [ ! -d "/home/${USER}/.local/share/bzu-gmb-utils/amdvlk_portable" ]
then
tput setaf 1; echo "Что то пошло не так и Драйвер Vulkan ${version_app} не установлен :(";tput sgr0
let "error += 10"
else
tput setaf 2; echo "Установка Драйвера Vulkan ${version_app} завершена :)"
tput sgr0
fi
else
tput setaf 1; echo "Драйвер Vulkan ${version_app} уже установлен в папку пользователя /home/${USER}/.local/share/bzu-gmb-utils, что бы не стереть ваши важные данные, установка прирывается!"
tput sgr0
fi

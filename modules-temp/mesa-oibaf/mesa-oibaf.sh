...
echo "${pass_user}" | sudo -S ppa-purge ppa:kisak/kisak-mesa
echo "${pass_user}" | sudo -S update -y || true
echo "${pass_user}" | sudo -S apt upgrade -y
echo "${pass_user}" | sudo -S add-apt-repository ppa:oibaf/graphics-drivers -y
echo "${pass_user}" | sudo -S update -y || true
echo "${pass_user}" | sudo -S apt upgrade -y
echo "${pass_user}" | sudo -S aptitude -y install libegl-mesa0:amd64 libgbm1:amd64 libgl1-mesa-dri:amd64 libglapi-mesa:amd64 libglu1-mesa:amd64 libglx-mesa0:amd64 libllvm14:amd64 libllvm14:i386 libxatracker2:amd64 mesa-utils  mesa-va-drivers:amd64 mesa-vdpau-drivers:amd64 mesa-vulkan-drivers:amd64 mesa-vulkan-drivers:i386 xserver-xorg-video-amdgpu libgl1-mesa-dri:i386 vulkan-tools || let "error += 1"
echo "${pass_user}" | sudo -S apt --fix-broken install -f -y
echo "${pass_user}" | sudo -S apt autoremove -y
echo "${pass_user}" | sudo -S apt autoclean -y

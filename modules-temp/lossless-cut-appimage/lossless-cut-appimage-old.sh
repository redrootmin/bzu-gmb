...
tput setaf 2; echo "Установка LosslessCut видеоредактора для обрезки\нарезки\добавления субтитров в аудио и видео файлах [https://github.com/mifi/lossless-cut]. Установка LosslessCut производиться в формате Appimage. Версия скрипта 1.0, автор: Яцына М.А."
tput sgr0
...
if [ ! -d "/home/${user_run_script}/lossless-cut" ]
then
tput setaf 2; echo "Видеоредактор ${version_app} не установлен в папку пользователя ${user_run_script}, поэтому можно устанавливать :)"
tput sgr0
cd
name_app_folder="lossless-cut"
name_app_arhive="${name_app_folder}.tar.xz"
name_script_start="lossless-cut_starter.sh"
rm -f ${name_app_arhive}
wget https://github.com/redrootmin/bzu-gmb-modules/releases/download/v1/lossless-cut.tar.xz
...
tput setaf 2; echo "Установка видеоредактора ${version_app} завершена :)"
tput sgr0
else
tput setaf 1; echo "Видеоредактор ${version_app} уже установлен в папку пользователя ${user_run_script}, что бы не стереть ваши важные данные, установка прирывается!"
tput sgr0
fi

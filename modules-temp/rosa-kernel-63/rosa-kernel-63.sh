...
echo "${pass_user}" | sudo -S mv /tmp/rosa-gaming-system-apps.repo /etc/yum.repos.d
echo "${pass_user}" | sudo -S dnf update -y
echo "${pass_user}" | sudo -S dnf autoremove -y
echo "${pass_user}" | sudo -S dnf clean packages
echo "${pass_user}" | sudo -S dnf install -y kernel-6.3-generic kernel-6.3-generic-devel kernel-6.3-rosa-flow-abi

#формируем информацию о том что в итоге установили и показываем в терминал
tput setaf 2;echo "В вашу систему установлены следующие linux ядра:";tput sgr0;rpm -qa | grep -w "kernel-6.3"

...
tput setaf 2; echo "Установка Simple Scan простой программы для сканирования [https://gitlab.gnome.org/GNOME/simple-scan]. Установка Simple Scan осуществлыется через официальный репозиторий Ubuntu. Версия скрипта 1.0, автор: Яцына М.А."
tput sgr0
...
echo "${pass_user}" | sudo -S apt install -f -y --reinstall simple-scan || let "error += 1"
#формируем информацию о том что в итоге установили и показываем в терминал
app_name="simple-scan"
...
simple-scan & sleep 5;echo "${pass_user}" | sudo -S killall simple-scan

...
tput setaf 2; echo "Установка Thunderbird бесплатной программы для работы с электронной почтой [https://www.thunderbird.net/].Установка Thunderbird осуществлыется через официальный репозиторий Ubuntu. Версия скрипта 1.0, автор: Яцына М.А."
tput sgr0

if echo "${linuxos_run_bzu_gmb}" | grep -ow "Ubuntu" > /dev/null || echo "${linuxos_run_bzu_gmb}" | grep -ow "Mint" > /dev/null
then
#запуск основных команд модуля
echo "${pass_user}" | sudo -S apt install -f -y --reinstall thunderbird thunderbird-locale-ru || let "error += 1"
fi

if echo "${linuxos_run_bzu_gmb}" | grep -ow "Debian GNU/Linux bookworm/sid" > /dev/null
then
echo "${pass_user}" | sudo -S apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys 0AB215679C571D1C8325275B9BDB3D89CE49EC21
echo "${pass_user}" | sudo -S chmod -Rf 777 /etc/apt/sources.list.d/;echo "${pass_user}" | sudo -S echo "deb http://ppa.launchpad.net/mozillateam/ppa/ubuntu focal main" > /etc/apt/sources.list.d/mozillateam-ppa.list;echo "${pass_user}" | sudo -S echo "deb-src http://ppa.launchpad.net/mozillateam/ppa/ubuntu focal main" >> /etc/apt/sources.list.d/mozillateam-ppa.list;echo "${pass_user}" | sudo -S chmod -Rf 755 /etc/apt/sources.list.d/
#запуск основных команд модуля
...
thunderbird & sleep 5;echo "${pass_user}" | sudo -S killall thunderbird

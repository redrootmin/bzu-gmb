...
tput setaf 2; echo "Установка Krita  популярного графического редактора с открытым исходным кодом [https://krita.org/]. Установка Krita осуществлыется через официальный репозиторий Ubuntu. Версия скрипта 1.0, автор: Яцына М.А."
tput sgr0
...
echo "${pass_user}" | sudo -S dnf install -y krita
...
krita & sleep 5;echo "${pass_user}" | sudo -S killall krita

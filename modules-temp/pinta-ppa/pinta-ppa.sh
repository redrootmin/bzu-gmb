...
tput setaf 2; echo "Установка Pinta - легковесный кроссплатформенный растровый графический редактор [https://www.pinta-project.com/]. Установка Pinta осуществлыется через официальный репозиторий Ubuntu. Версия скрипта 1.0, автор: Яцына М.А."
tput sgr0

#запуск основных команд модуля
echo "${pass_user}" | sudo -S add-apt-repository -y  ppa:pinta-maintainers/pinta-daily || let "error += 1"
echo "${pass_user}" | sudo -S apt install -f -y --install pinta || let "error += 1"
...
pinta & sleep 5;echo "${pass_user}" | sudo -S killall pinta

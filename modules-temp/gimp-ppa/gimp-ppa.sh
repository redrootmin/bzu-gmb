...
tput setaf 2; echo "Установка GIMP - свободно распространяемый графический редактор [https://www.gimp.org/]. Установка GIMP осуществлыется через официальный репозиторий Ubuntu. Версия скрипта 1.0, автор: Яцына М.А."
tput sgr0

#запуск основных команд модуля
#echo "${pass_user}" | sudo -S add-apt-repository -y  ppa:inkscape.dev/stable || let "error += 1"
echo "${pass_user}" | sudo -S apt install -f -y --reinstall gimp || let "error += 1"
...
gimp & sleep 5;echo "${pass_user}" | sudo -S killall gimp

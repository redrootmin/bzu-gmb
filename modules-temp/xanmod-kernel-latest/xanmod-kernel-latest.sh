...
if echo "${linuxos_run_bzu_gmb}" | grep -ow "Ubuntu 20.04.4 LTS" > /dev/null || echo "${linux_os}" | grep -ow "Linux Mint 20" > /dev/null || echo "${linux_os}" | grep -ow "Ubuntu 21.10" > /dev/null || echo "${linux_os}" | grep -ow "Ubuntu 22.04 LTS" > /dev/null
then

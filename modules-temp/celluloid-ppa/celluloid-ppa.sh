...
tput setaf 2; echo "Установка Celluloid простого интерфейса GTK+ для плеера mpv [https://celluloid-player.github.io/]. Установка Celluloid осуществлыется через оригинальный репозиторий плеера. Версия скрипта 1.0, автор: Яцына М.А."
tput sgr0

#запуск основных команд модуля
echo "${pass_user}" | sudo -S add-apt-repository -y  ppa:xuzhen666/gnome-mpv || let "error += 1"
echo "${pass_user}" | sudo -S apt update -y
echo "${pass_user}" | sudo -S apt install -f -y --reinstall celluloid || let "error += 1"
...
celluloid & sleep 5;echo "${pass_user}" | sudo -S killall celluloid

...
tput setaf 2; echo "Установка Key Mapper простой утилиты для назначения(mapping) кнопок вашего геймпада, мышки, клавиатуры на ввод новых значений\команд [https://github.com/sezanzeb/key-mapper].Установка Key Mapper осуществляется с помощью deb-пакета, скаченого с официальной страницы проекта. Версия скрипта 1.0b, автор: Яцына М.А."
tput sgr0
...
#запуск основных команд модуля
cd
wget https://github.com/sezanzeb/key-mapper/releases/download/1.2.1/key-mapper-1.2.1.deb
echo "${pass_user}" | sudo -S apt install -f -y ./key-mapper-1.2.1.deb || let "error += 1"
echo "${pass_user}" | sudo -S rm -f "key-mapper-1.2.1.deb" || let "error += 1"
fi
...
echo "${pass_user}" | sudo -S key-mapper-gtk & sleep 5;echo "${pass_user}" | sudo -S killall "key-mapper-gtk"

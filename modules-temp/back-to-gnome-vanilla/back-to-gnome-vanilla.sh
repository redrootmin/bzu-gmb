...
echo "${pass_user}" | sudo -S apt install -f -y --reinstall gnome-session ubuntu-gnome-default-settings plymouth-theme-ubuntu-gnome-logo numix-icon-theme-circle vanilla-gnome-default-settings ubuntu-gnome-desktop gnome-tweaks libglib2.0-dev grub-customizer psensor inxi;sudo apt -y remove ubuntu-session;gsettings set org.gnome.desktop.interface gtk-theme Adwaita-dark;gsettings set org.gnome.desktop.interface icon-theme Numix-Circle;gsettings set org.gnome.desktop.wm.preferences button-layout ":minimize,maximize,close";wget https://github.com/redrootmin/bzu-gmb-modules/releases/download/v1/ubuntu-logo-icon.png;sudo cp -f "${script_dir}/modules-temp/${name_script}/temp/ubuntu-logo-icon.png" "/usr/share/icons/hicolor/256x256/apps"; sudo rm -f ubuntu-logo-icon.png;gsettings set org.gnome.desktop.background picture-uri file:////usr/share/backgrounds/ubuntu-gnome/sochi_krasnaya_polyana_mountains_by_alexander_lyubavin.jpg
cd
echo "${pass_user}" | sudo -S rm -r "${script_dir}/modules-temp/${name_script}/temp" || let "error += 1"
...
#nome-extensions enable caffeine@patapon.info
...
GTK_THEME="Adwaita-dark" yad --title="Back to Ubuntu Vanilla" --image-on-top --picture --size=fit --filename="${script_dir}/icons/ubuntu-logo-icon.png" --width=327 --height=327 --center --inc=256  --text-align=center --text="ТРЕБУЕТСЯ ПЕРЕАГРУЗКА СИСТЕМЫ!" --timeout=5 --timeout-indicator=bottom 
exit 0
#sudo reboot

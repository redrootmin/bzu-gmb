...
tput setaf 2; echo "Установка QPDF Tools бесплатной программы с открытым исходным кодом, с возможностью сжатия, разделения, объединения и поворота PDF-документов [https://github.com/silash35/qpdftools/wiki]. Установка QPDF Tools осуществлыется через оригинальный репозиторий программы. Версия скрипта 1.0, автор: Яцына М.А."
tput sgr0

#запуск основных команд модуля
echo "${pass_user}" | sudo -S add-apt-repository -y  ppa:silash35/qpdftools || let "error += 1"
echo "${pass_user}" | sudo -S apt update -y
echo "${pass_user}" | sudo -S apt install -f -y --reinstall qpdftools || let "error += 1"
...
qpdftools & sleep 5;echo "${pass_user}" | sudo -S killall qpdftools
